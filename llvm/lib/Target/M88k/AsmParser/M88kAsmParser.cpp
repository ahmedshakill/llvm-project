...
class M88kOperand : public llvm::MCParsedAsmOperand{
    enum OperandKind{
      OpKind_Token,
      OpKind_Reg,
      OpKind_Imm,
...
// Parse Register of form %(r|x|cr|fcr)<No>
...
bool M88kAsmParser::parseRegister(MCRegister &RegNo, 
                                  SMLoc &StartLoc,
                                  SMLoc &EndLoc){
  return parseRegister(RegNo, StartLoc, EndLoc, false);
}

ParseStatus M88kAsmParser::tryParseRegister(MCRegister &RegNo, 
                        SMLoc &StartLoc, SMLoc &EndLoc){
  bool Result = 
    parseRegister(RegNo, StartLoc, EndLoc, true);

  bool PendingErrors = getParser().hasPendingError();
  getParser().clearPendingErrors();
  if(PendingErrors)
    return llvm::MatchOperand_ParseFail;
  if(Result)
    return llvm::MatchOperand_NoMatch;
  return llvm::MatchOperand_Success;
}
